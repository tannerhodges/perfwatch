...
import persist from '../vendor/alpinejs-persist-module.esm-d352364.js';
import {
	Chart,
	CategoryScale,
	LinearScale,
...
type EventsGroupedByInstanceIdEntry = {
	instanceId: string;
	events: LogData[];
};

type EventsGroupedByInstanceId = EventsGroupedByInstanceIdEntry[];
...
Alpine.data('App', function () {
	return {
		version: (Alpine as AlpinePersist).$persist('').as('version'),
		events: (Alpine as AlpinePersist).$persist([]).as('events'),
		fileDiffs: (Alpine as AlpinePersist).$persist([]).as('fileDiffs'),
		folderPath: (Alpine as AlpinePersist).$persist('').as('folderPath'),
		selectedMetric: (Alpine as AlpinePersist).$persist('').as('selectedMetric'),
